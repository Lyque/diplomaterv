...
A várhatóan leggyorsabban lefutó taszk kerül futási állapotba ütemezés bekövetkezésekor. Helyes mûködés esetén az átlagos várakozási idõ optimális lesz., viszont a gyakorlatban nehéz elõre megjósolni egy taszk futási idejét.
...
A következõ taszk kiválasztása a prioritása alapján történik. Növelni lehet a hatékonyságát más metódusok egyidejû használatával (például a prioritást a várható futási idejébõl határozzuk meg; a prioritást növeljük az idõ elteltével; a prioritást csökkentjük az eddigi futó állapotban töltött idõ arányában).
...
A felsorolt ütemezési elveket akár keverve is lehet alkalmazni. Ilyen ütemezési mechanizmus például a \emph{multilevel queue}, ahol minden sor saját ütemezési algoritmussal rendelkezik, és egy külön algoritmus felel az egyes sorok arbitrációjáért (\figref{05_multilevel_queue_scheduling}a).
...
A számláló típusú szemafor minden jelzéskor növeli az értékét. Ekkor (amíg el nem éri a maximális értékét) nem kerül \emph{Blokkolt} állapotba a jelzõ taszk. A számláló szemafor felfogható úgy, mint egy egynél több adat tárolására képes sor (\figref{08_counting_semaphore}a), melynek nem vizsgáljuk az értékét, csak azt, hogy éppen tartalmaz-e még adatot vagy sem.
...
\begin{description}
\item[$\bullet$\ Események számlálása:] ekkor minden esemény hatására növeljük a szemafor értékét (új elemet helyezünk a sorba). A szemafor aktuális értéke a beérkezett és a feldolgozott események különbsége. A számlálásra használt szemafor inicializálási értéke nulla.
